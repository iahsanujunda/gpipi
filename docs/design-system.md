...
- The line's exact active window as a compact chip, for example `WEEKLY · 20–26 JUL` or `MONTHLY · 24 JUL – 24 AUG`. Monthly windows run payday-to-payday: the 25th, or the preceding Friday when the 25th is on a weekend. `windowEndExclusive` is never displayed as included, so the visible final date is one day before it. A range crossing a month boundary names both endpoints with a spaced en dash, such as `29 JUN – 5 JUL`, and remains on one line in a selectively wider period control.

...
- **Available, not applied.** On a current-period mobile card, place one quiet outlined action below utilization: `Add ¥… from last week/payday period` or `Subtract ¥… from last week/payday period`, with a minimum `44 px` height. Do not wrap it in a tinted carry card or add `PENDING`, unused/overrun labels, or instructional copy.
- **Review.** The action opens the shared `AnimatedBottomSheet` on phones and its centered-dialog adaptation from medium widths upward. Keep it decision-complete but terse: budget and target window, source window with source spend versus allowance, current allowance, signed prior-period amount, new allowance, and Back plus Add/Subtract action. Do not add generic explanatory prose.
...
- **Applied.** On success, invalidate the relevant spend queries, close the confirmation surface, announce `¥… added to/subtracted from …`, remove the action, and retain at most one subdued `Included ±¥… from last week/payday period` note. Utilization immediately uses the effective allowance; do not add an `APPLIED` chip or a second breakdown card.

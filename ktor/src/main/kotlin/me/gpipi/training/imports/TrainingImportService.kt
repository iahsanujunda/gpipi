...
class TrainingImportService(
    ...
    suspend fun chooseWeek(
        ...
        val workouts = dbQuery(db) { repository.workouts(ownerUserId, header.programId) }
    ...
    suspend fun saveMapping(
        ...
        val workoutIds = dbQuery(db) { repository.workouts(ownerUserId, header.programId) }.map { it.id }.toSet()

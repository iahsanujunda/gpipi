...
class TrainingImportMigrationTest : PersistenceTest() {
    ...
    @Test
    fun `new program import creates and activates the program only during apply`() = runBlocking {
        val owner = "U-NEW-PROGRAM-IMPORT"
        val training = TrainingService(db, TrainingRepository())
        val previousProgramId = assertIs<ProgramCreateResult.Created>(
            training.createProgram(
                owner,
                ProgramAuthoringInput(
                    name = "M1",
        ...
        val repository = TrainingImportRepository()
        val now = OffsetDateTime.of(2026, 8, 11, 0, 0, 0, 0, ZoneOffset.UTC)
        val importId = createReviewedNewProgramImport(owner, repository, now)

        val beforeApply = transaction(db) {
            exec("select count(*) from program where owner_user_id = '$owner'") { rs -> rs.next(); rs.getInt(1) }
        }
        assertEquals(1, beforeApply)

        val appliedWeek = dbQuery(db) {
            val header = requireNotNull(repository.header(owner, importId))
            val tab = repository.tabs(importId).single()
            val week = repository.weeks(importId).single()
            val draft = json.decodeFromString<TrainingPrescriptionExtraction>(requireNotNull(week.extractedDraft))
            val matches = repository.matches(importId).associateBy { it.sourceMovementKey }
            repository.apply(owner, header, listOf(AppliedImportWorkout(tab, week, draft, matches)), now, json)
        }
        ...
        val result = transaction(db) {
            exec(
                """
                select
                    count(*) filter (where active) as active_programs,
                    max(name) filter (where active) as active_name,
                    bool_and(not active) filter (where id = '$previousProgramId') as prior_inactive,
                    (select count(*) from workout_week ww
                     join workout w on w.id = ww.workout_id
                     join program np on np.id = w.program_id
                     where np.owner_user_id = '$owner' and np.name = 'M2' and ww.week_number = 5) as imported_weeks,
                    (select count(*) from training_session) as sessions,
                    (select count(*) from performed_exercise) as performed_exercises,
                    (select count(*) from performed_set) as performed_sets
                from program
                ...
                rs.next()
                listOf(
                    rs.getInt("active_programs"), rs.getString("active_name"), rs.getBoolean("prior_inactive"),
                    rs.getInt("imported_weeks"), rs.getInt("sessions"),
                    rs.getInt("performed_exercises"), rs.getInt("performed_sets"),
                )
            }
        }
        assertEquals(listOf(1, "M2", true, 1, 0, 0, 0), result)
    }

    @Test
    fun `cancelling a new program import leaves the training domain empty`() = runBlocking {
        val owner = "U-CANCEL-NEW-PROGRAM"
        val repository = TrainingImportRepository()
        val now = OffsetDateTime.of(2026, 8, 11, 0, 0, 0, 0, ZoneOffset.UTC)
        val importId = dbQuery(db) {
            repository.createReadingNewProgramImport(owner, "selected-sheet", now).also { id ->
                repository.completeDiscovery(
                    id,
                    "JUNDA – M2",
                    "M2",
                    listOf(SheetTabGrid(101, "Full Body 1", 1, 100, 20, emptyList())),
                ...
                repository.saveNewProgramDraft(id, "M2", null, null, now)
                repository.cancel(id, now)
            }
        }
        ...
        val result = transaction(db) {
            exec(
                """
                select ti.state, ti.program_id, (select count(*) from program where owner_user_id = '$owner') as programs
                from training_import ti where ti.id = '$importId'
                """.trimIndent(),
            ) { rs ->
                rs.next()
                Triple(rs.getString("state"), rs.getObject("program_id"), rs.getInt("programs"))
            }
        }
        assertEquals(Triple("CANCELLED", null, 0), result)
    }

    private suspend fun createReviewedNewProgramImport(
        owner: String,
        ...
        val importId = dbQuery(db) {
            repository.createReadingNewProgramImport(owner, "selected-sheet", now).also { id ->
                ...
                repository.saveNewProgramDraft(id, "M2", "Pregnancy strength block", null, now)
                repository.chooseWeek(id, 5, now)
                val tab = repository.tabs(id).single()
                repository.saveMapping(
                    id,
                    5,
                    listOf(
